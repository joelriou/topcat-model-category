...
lemma continuousGeneratedBy_iff_uncurry [∀ i, LocallyCompactSpace (X i)]
    (g : Y → ContinuousMapGeneratedBy X Z T) :
    ContinuousGeneratedBy X g ↔
      ∀ (i₁ : ι) (f₁ : C(X i₁, Y)) (i₂ : ι) (f₂ : C(X i₂, Z)),
        Continuous (ContinuousMap.toFun ∘ precomp f₂ ∘ g ∘ f₁).uncurry := by
...
lemma continuousGeneratedBy_dom_prod_iff [∀ i j, IsGeneratedBy X (X i × X j)]
  ...
  · rw [continuousGeneratedBy_def]
    intro i f
    exact (h i (ContinuousMap.fst.comp f) i (ContinuousMap.snd.comp f)).comp
...
def curryEquiv :
  ContinuousMapGeneratedBy X (Y × Z) T ≃
    ContinuousMapGeneratedBy X Y (ContinuousMapGeneratedBy X Z T) where
  toFun g :=
    { toFun y := g.comp ⟨_, (Continuous.prodMk_right y).continuousGeneratedBy⟩
      prop := by simpa only [continuousGeneratedBy_iff_uncurry,
