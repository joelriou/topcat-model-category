...
instance : HasFunctorialFactorization (trivialCofibrations SSet.{u}) (fibrations SSet) := by
  ...
  · trans (trivialCofibrations TopCat).inverseImage SSet.toTop
    · simp only [retracts_le_iff]
      ...
      have : (coproducts.{u} J).pushouts ≤
        ...
        let hc₂' : IsColimit c₂' := isColimitOfPreserves toTop hc₂
        have : hc₁'.desc { pt := _, ι := Functor.whiskerRight g toTop ≫ c₂'.ι } =
          toTop.map (hc₁.desc { pt := _, ι := g ≫ c₂.ι }) := by
            apply hc₁'.hom_ext
            rintro ⟨j⟩
            rw [IsColimit.fac]
            dsimp [c₁']
